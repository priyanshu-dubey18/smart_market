...
async function sendOTP() {
  ...
  try {
    const res = await fetch('/api/method/smart_market.api.auth.send_otp', {
      ...
      body: JSON.stringify({ medium: 'email', contact: email.value, purpose: 'reset-password' })
    })
    const data = await res.json()
    if (!res.ok) throw new Error(data.message || 'Failed to send OTP')

    setSignupData({ otpFlow: 'reset-password', otpContact: email.value, otpMedium: 'email', email: email.value })
    ...
    <div class="auth-container">
      ...
      <div class="auth-card">
        ...
        <div class="card-top">
          ...
          <p>Enter your registered email address. We'll send an OTP to verify your identity before resetting your password.</p>
        </div>
        <form @submit.prevent="sendOTP" class="auth-form">
          <div class="field-group">
            ...
            <span class="field-hint">OTP will be sent to this email</span>
          </div>
          <div v-if="error" class="error-box">⚠️ {{ error }}</div>
          <button type="submit" class="submit-btn" :disabled="loading">
            <span v-if="!loading">Send OTP to Email →</span>
            <span v-else class="spinner"></span>
          </button>
        </form>
        <div class="alt-reset">
          <span>Want to reset email instead?</span>
          <button class="link-btn" @click="router.push('/auth/reset-email')">Reset Email</button>
        </div>
      </div>
    </div>
  </div>
...
.orb-2 { width: 300px; height: 300px; background: #6366f1; bottom: -50px; right: 10%; animation: orbFloat 10s ease-in-out infinite reverse; }
...
.submit-btn:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 0 35px rgba(245,158,11,0.5); }
...
.link-btn { background: none; border: none; color: #f59e0b; font-weight: 600; cursor: pointer; font-size: 0.85rem; }
